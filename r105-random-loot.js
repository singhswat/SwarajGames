(()=>{
'use strict';
if(window.__IRONTRAP_R105_LOOT)return;window.__IRONTRAP_R105_LOOT=true;
...
function randomEnemyBonus(e,d){
  if(!e||!d||d.isBoss||e.__r101Drop||e.__r105Bonus)return;e.__r101Drop=true;e.__r105Bonus=true;
...
function bossLoot(e,d){
  if(!e||!d||!d.isBoss||e.__r105BossLoot)return;e.__r105BossLoot=true;
...
if(typeof killEnemy==='function'&&!killEnemy.__r105){
  const old=killEnemy;
  const wrapped=function(e){
    ...
    return out;
  };wrapped.__r105=true;try{killEnemy=wrapped;}catch(_){window.killEnemy=wrapped;}
}
if(typeof takePickup==='function'&&!takePickup.__r105){
  const old=takePickup;
  const wrapped=function(pk){
    const kind=pk&&pk.kind,was=!!(pk&&pk.taken),x=pk?pk.x:0,y=pk?pk.y:0;
    const out=old.apply(this,arguments);
    if(!pk||was||pk.__r105ChestBonus)return out;
    if(kind==='chest'||kind==='goldchest'){
      pk.__r105ChestBonus=true;
    ...
    return out;
  };wrapped.__r105=true;try{takePickup=wrapped;}catch(_){window.takePickup=wrapped;}
}
})();
